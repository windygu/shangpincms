    ...
    jQuery.fn.imageMaps = function(setting) {
		...
		$container.each(function(){
			...
			$images.each(function(){
				...
				_img_conrainer.append('<div class="button-conrainer"><input  type="button" value="添加热点链接" /></div><br/>').append('<div class="link-conrainer"></div>').append($.browser.msie ? $('<div class="position-conrainer" style="position:absolute"></div>').css({
					background:'#fff',
					opacity:0
				...
				if(map_name !=''){
					...
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						...
						_link_conrainer.append('<p ref="' + index + '" class="map-link"><span class="link-number-text">链接 ' + index + '</span>: <input type="text"  name="link' + image_param + index + '" value="' + $(this).attr('href') + '" /><input type="hidden" class="rect-value" name="rect' + image_param + index + '" value="' + coords + '" /><br /><span class="link-number-text">链接 ' + index + '标题</span>: <input type="text"  name="title' + image_param + index + '" value="' + $(this).attr('title') + '" /></p>');
						coords = coords.split(',');
		...
		$('.button-conrainer input[type=button]').click(function(){
			...
			if (index > 15)
			{ alert("热点不能超过15个"); return false;}
			var image = $(this).parent().parent().find('img[ref=imageMaps]').attr('name');
			image = (image == '' ? '' : '['+ image + ']');
			_link_conrainer.append('<p ref="' + index + '" class="map-link"><span class="link-number-text">链接 ' + index + '</span>: <input type="text"  name="link' + image + index + '" value="" /><input type="hidden" class="rect-value" name="rect' + image + index + '" value="10,10,100,40" /><br /><span class="link-number-text">链接 ' + index + '标题</span>: <input type="text"  name="title' + image + index + '" value="" /></p>');
			_position_conrainer.append('<div ref="'+index+'" class="map-position" style="left:10px;top:10px;width:200px;height:130px;"><div class="map-position-bg"></div><span class="link-number-text">Link '+index+'</span><span class="delete">X</span><span class="resize"></span></div>');
			bind_map_event();
			define_css();
		});
		...
		function bind_map_event(){
			...
			$('.position-conrainer .map-position .delete').unbind('click').click(function(){
				...
				_link_conrainer.find('.map-link').each(function(){
				    $(this).attr('ref', index).find('.link-number-text').html('链接 ' + index);
					index ++;
				});
				index = 1;
		...
		function define_css(){
			...
			$container.find('.button-conrainer input[type=button]').css({
			    display: 'block',
			    height: 40,
                'margin-top':'10px'
			});
			$container.find('.map-link').css({
			    display: 'block',
			});
			$container.find('.link-conrainer input[type=text]').css({			   
			    'height': "28px",
			    'line-height': '28px',
			    width: 300,                
			    'margin-top': '5px'
