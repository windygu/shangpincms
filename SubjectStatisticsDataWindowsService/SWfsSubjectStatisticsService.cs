...
using System.Linq;
using System.Text;
using SubjectStatisticsDataWindowsService.DBUtility;
...
namespace SubjectStatisticsDataWindowsService
{
    public class SWfsSubjectStatisticsService
    {
        ...
        public void GetSubjectDataList(int type)
        {
            ...
            sql.Append("SELECT sub.[SubjectNo] ");
            ...
            try
            {
                ...
                {
                    SqlDataReader reader = SqlHelper.ExecuteReader(conn, CommandType.Text, sql.ToString(), param);
                    if (reader != null)
                    {
                        while (reader.Read())
                        {
        ...
        private void SetRedisCache(string caheKey, List<SubjectSaleVisitStatisticsDataModel> list)
        {
            if (!String.IsNullOrEmpty(caheKey))
            {
                RedisCacheProvider.Instance.Set<List<SubjectSaleVisitStatisticsDataModel>>(caheKey, list, new TimeSpan(0, 60, 0));
            }
        }
        #endregion
